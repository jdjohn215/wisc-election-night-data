...
read_ashland <- function(workbookpath, save_output = T){
  ...
  read_sheet <- function(sheetindex){
    sheet <- read_excel(workbookpath, sheet = sheetindex, col_types = "text",
  ...
  all.output <- map(.x = 1:length(sheet.list),
    ...
    mutate(county = "Ashland",
           ...
           municipality = word(municipality, 1, sep = "\\bW[0-9]|\\bWD|\\bWARD|\\bD[0-9]|\\bW\\b"))
...
read_barron <- function(workbookpath, save_output = T){
  ...
  read_sheet <- function(sheetindex){
    read_excel(workbookpath, sheet = sheetindex, col_names = F,
               .name_repair = "unique_quiet") |>
      rename(candidate = 1) |>
      mutate(candidate = zoo::na.locf(candidate),
             contest = sheetvector[sheetindex]) |>
      select(contest, candidate, reporting_unit = ...3, votes = ...4) |>
      filter(reporting_unit != "Precinct")
  }
  
  all.output <- map(.x = 1:length(sheetvector),
                    .f = read_sheet) |>
    list_rbind() |>
    type_convert() |>
...
read_bayfield <- function(workbookpath, sheetvector, save_output = T){
 ...
 all.output <- map(.x = sheetvector,
   ...
   mutate(county = "Bayfield",
          across(where(is.character), str_to_upper),
          ctv = case_when(
            str_detect(reporting_unit, "CITY OF|^C\\b") ~ "C",
            str_detect(reporting_unit, "VILLAGE OF|^V\\b") ~ "V",
            str_detect(reporting_unit, "TOWN OF|^T\\b") ~ "T",
            TRUE ~ "T"
          ),
          municipality = str_remove(reporting_unit, "\\s*\\([^\\)]+\\)"), # remove parentheticals containing CTV
          municipality = str_remove(municipality, "^C\\b|^T\\b|^V\\b"),
          municipality = word(municipality, 1, sep = "\\bW[0-9]|\\bWD|\\bWARD|\\bD[0-9]"),
          municipality = str_remove_all(municipality, coll(".")),
          contest = word(contest, 1, sep = "_"),
          across(where(is.character), str_squish)) |>
   filter(str_detect(municipality, "TOTALS", negate = T)) |>
...
read_vernon <- function(workbookpath, sheetno = 2, save_output = T){
  ...
  dtemp <- readxl::read_excel(workbookpath, col_types = "text", sheet = sheetno,
    ...
    select(-`Vernon County_November 5, 2024`) |>
    pivot_longer(cols = -c(reporting_unit),
                 names_to = "contestcandidate", values_to = "votes") |>
    mutate(contestcandidate = str_replace_all(str_squish(contestcandidate), coll("\r\n"), " ")) |>
...
read_waushara <- function(workbookpath, sheetvector, save_output = T){
  process_sheet <- function(sheetindex){
    read_excel(workbookpath, sheet = sheetindex, col_names = T, col_types = "text") |>
      ...
      pivot_longer(cols = -reporting_unit, names_to = "contestcandidate", values_to = "votes") |>
      filter(!is.na(votes)) |>
      separate(contestcandidate, into = c("contest", "candidate"), sep = "\\n")
  }
  ...
  dtemp <- map(.x = sheetvector,
    ...
    mutate(county = "Waushara",
           ...
           across(where(is.character), str_squish),
           contest = if_else(contest == "PRESIDENT", "PRESIDENT/ VICE PRESIDENT", contest)) |>
    filter(str_detect(candidate, coll("#"), negate = T),
           ! contest %in% c("PERCENTAGE VOTER TURNOUT", "# OF OUTSTANDING"),
...
read_oconto <- function(workbookpath, save_output = T){
  ...
  headers <- c("candidate", "T Abrams W1-3", "T Bagley W1", "T Brazeau W1-3", "T Breed W1", "T Chase W1-5",
    ...
    "T Lt River W1-2", "T Lt Suamico W1-8", "T Maple Valley W1", "drop1", "T Morgan W1-2", "drop2",
    "T Mountain W1", "T Oconto W1-3", "T Oconto Falls W1-2", "T Pensaukee W1-2", "drop3",
    "T Riverview W1-2", "drop4", "T Spruce W1-2", "T Stiles W1-2", "T Townsend W1",
    "T Underhill W1", "V Lena W1", "V Pulaski W5", "V Suring W1", "C Gillett W1-3",
    "C Oconto W1-7", "C Oconto Falls W1-6", "TOTALS")
  headers2 <- c("candidate", "T Abrams W1-3", "T Bagley W1", "T Brazeau W1-3", "T Breed W1", "T Chase W1-5",
               "T Doty W1", "T Gillett W1-2", "T How W1-2", "T Lakewood W1", "T Lena W1",
  ...
  sheet2 <- readxl::read_excel(workbookpath, col_names = headers2, sheet = 6,
                               col_types = "text", .name_repair = "unique_quiet") |>
    mutate(contest = if_else(row_number() == 1 | lag(candidate, 1) == "Undervotes",
                             true = candidate, false = NA),
           contest = zoo::na.locf(contest)) |>
  ...
  sheet3 <- readxl::read_excel(workbookpath, col_names = headers2, sheet = 7,
                               col_types = "text", .name_repair = "unique_quiet") |>
    mutate(contest = if_else(row_number() == 1 | lag(candidate, 1) == "Undervotes",
                             true = candidate, false = NA),
           contest = zoo::na.locf(contest)) |>
...
read_richland <- function(workbookpath, sheetvector = 1:2, save_output = T){
  ...
  read_sheet <- function(sheetindex){
    ...
    office <- sheet$...1[3]
    office
    start.row <- min(which(str_detect(sheet$...1, "Akan")))
    start.header.row <- min(which(sheet$...1 == "Party"))
    ...
    sheet |>
      ...
      mutate(contestcandidate = str_replace_all(str_squish(contestcandidate), coll("\r\n"), " ")) |>
      separate(contestcandidate, sep = "_(?!.*_)", into = c("contest", "candidate")) |>
      mutate(contest = word(contest, 1, sep = "_")) |>
      filter(!is.na(votes))
  }
...
read_chippewa <- function(workbookpath, save_output = T){
  sheetlist <- readxl::excel_sheets(workbookpath)
  
  process_sheet <- function(sheetindex){
    sheet <- readxl::read_excel(workbookpath, sheet = sheetindex, col_names = T,
                                .name_repair = "unique_quiet")
    sheet |>
      select(-any_of(c("reported", "drop"))) |>
      pivot_longer(cols = -c(contest, reporting_unit), names_to = "candidate", values_to = "votes")
  }
  
  dtemp <- map(.x = sheetlist,
      .f = process_sheet) |>
    list_rbind() |>
    type_convert() |>
...
################################################################################
read_trempealeau <- function(workbookpath, sheetvector = 1:3, save_output = T){
  ...
  process_sheet <- function(sheetindex){
    ...
    colnames <- sheet |>
      ...
      pivot_longer(cols = -rownum) |>
      pivot_wider(names_from = rownum, values_from = value) |>
      mutate(`2` = zoo::na.locf(`2`, na.rm = F),
             header = if_else(str_detect(header1, "President"), header1, NA)) |>
      select(-name) |>
    ...
    sheet |>
      ...
      mutate(across(where(is.character), str_squish)) |>
      separate(contest, into = c("contest", "candidate"), sep = "_") |>
      mutate(votes = str_remove_all(votes, "X"),
             votes = na_if(votes, "")) |>
      filter(!is.na(votes))
  }
...
################################################################################
read_crawford <- function(workbookpath, sheetvector = 1:2, save_output = T){
  ...
  read_sheet <- function(sheetindex){
    ...
    dtemp <- sheet |>
      ...
      mutate(ctvassign = case_when(
        reporting_unit == "Bell Center" ~ "VILLAGE OF",
        reporting_unit == "1st Ward" ~ "CITY OF PRAIRIE DU CHIEN",
      ...
      mutate(county = "Crawford",
             ...
             reporting_unit = str_replace(reporting_unit, "PRA DU CHN", "PRAIRIE DU CHIEN"),
             reporting_unit = case_when(
...
read_burnett <- function(workbookpath, save_output = T){
  ...
  dtemp <- sheet1.1 |>
    ...
    mutate(county = "Burnett",
           ...
           municipality = str_remove(municipality, coll(",")),
           across(where(is.character), str_squish)) |>
    filter(str_detect(municipality, "\\bPAGE\\b", negate = T))
...
################################################################################
read_monroe <- function(workbookpath, save_output = T){
  sheet.orig <- read_excel(workbookpath, col_names = F, .name_repair = "unique_quiet")
  ...
  colnames <- sheet.orig |>
    ...
    pivot_longer(cols = -rownum) |>
    pivot_wider(names_from = rownum, values_from = value) |>
    unite("office", `2`, `3`, na.rm = T) |>
    mutate(office = na_if(office, ""),
           office = zoo::na.locf(office, na.rm = F),
           across(where(is.character), ~str_replace(.x, coll("\n"), " "))) |>
    unite("colname", `1`, office, `4`, na.rm = T) |>
    mutate(colname = str_replace_all(colname, coll("\n"), " "),
  ...
  sheet <- sheet.orig |>
    ...
    rename(reporting_unit = 1) |>
    filter(!is.na(reporting_unit)) |>
    select(-c(contains("OFFICE_Monroe"), contains("Voter"))) |>
    mutate(ctv = if_else(reporting_unit %in% c("TOWNS", "CITIES", "VILLAGES"), str_sub(reporting_unit, 1, 1), NA),
           ctv = zoo::na.locf(ctv)) |>
    select(ctv, everything()) |>
    filter(! reporting_unit %in% c("TOWNS", "CITIES", "VILLAGES")) |>
    pivot_longer(cols = -c(ctv, reporting_unit), names_to = "contestcandidate", values_to = "votes") |>
    separate(contestcandidate, sep = "_(?!.*_)", into = c("contest", "candidate")) |>
    mutate(county = "Monroe",
           across(where(is.character), str_to_upper),
           reporting_unit = str_remove_all(reporting_unit, coll(".")),
           municipality = word(reporting_unit, 1, sep = " [0-9]"),
           across(where(is.character), str_squish),
           reporting_unit = paste(ctv, municipality, "WARDS",
                                  str_remove(reporting_unit, municipality)),
           across(where(is.character), str_squish)) |>
    filter(candidate != "OUTSTANDING PROVISIONAL")
  
  
  if(save_output == TRUE){
    write_csv(sheet, paste0("2024-nov/raw-processed/", str_remove(word(workbookpath, -1, sep = "/"), ".pdf|.xlsx"), ".csv"))
  }
  sheet
}
################################################################################
...
################################################################################
read_rusk <- function(workbookpath, sheetvector = 1:2, save_output = T){
  ...
  dtemp <- map(.x = 1:length(sheetvector),
               .f = read_sheet) |>
    list_rbind() |>
    type_convert() |>
    mutate(county = "Rusk",
           ...
           ctv = str_sub(reporting_unit, 1, 1),
           reporting_unit = str_remove_all(reporting_unit, coll(".")),
           municipality = str_remove(reporting_unit, "^T[/]|^C[/]|^V[/]|TOWN OF |VILLAGE OF |CITY OF |VILAGE OF "),
           municipality = word(municipality, 1, sep = coll(","))) |>
    filter(municipality != "TOTALS")
...
################################################################################
read_price<- function(workbookpath, sheetvector = 1, save_output = T){
  
  read_sheet <- function(sheetindex){
    sheet <- read_excel(workbookpath, sheet = sheetindex, col_names = F,
                        col_types = "text", .name_repair = "unique_quiet")
    colnames <- str_remove_all(unlist(sheet[1,]), coll("\n"))
    
    dtemp <- sheet |>
      filter(row_number() > 1) |>
      set_names(colnames) |>
      rename(candidate = 1) |>
      filter(str_detect(candidate, "^Reporting|^Number|^Percentage", negate = T)) |>
      mutate(contest = if_else(is.na(`T O T A L`), candidate, NA),
             contest = zoo::na.locf(contest)) |>
      select(contest, everything()) |>
      filter(contest != candidate) |>
      select(-`T O T A L`) |>
      pivot_longer(cols = -c(contest, candidate), names_to = "reporting_unit", values_to = "votes") |>
      mutate(reporting_unit = str_remove_all(reporting_unit, " "),
             reporting_unit = case_when(
               reporting_unit == "ParkFalls" ~ "City of Park Falls",
               reporting_unit == "Phillips" ~ "City of Phillips",
               reporting_unit == "VCatawba" ~ "Village of Catawba",
               reporting_unit == "TCatawba" ~ "Town of Catawba",
               reporting_unit == "VKennan" ~ "Village of Kennan",
               reporting_unit == "VPrentice" ~ "Village of Prentice",
               reporting_unit == "Fifield1" ~ "Town of Fifield Ward 1",
               reporting_unit == "Fifield2" ~ "Town of Fifield Ward 2",
               reporting_unit == "TKennan" ~ "Town of Kennan",
               reporting_unit == "TPrentIce" ~ "Town of Prentice",
               TRUE ~ paste("Town of", reporting_unit)
             ))
  }
...
################################################################################
read_waupaca <- function(workbookpath, save_output = T){
  sheetvector <- excel_sheets(workbookpath)
